...
function wprc_add_report(){
	...
	if($permissions['login_required'] && !is_user_logged_in()){
		$message['message'] = '需要先 <a href="<?php echo wp_login_url(); ?>" title="Login">登录</a> ';
	...
	if(wprc_is_spam($new_report)){
		$message['message'] = '你的举报被网站过滤器标记为垃圾举报，请修改举报内容';
		die(json_encode($message));
	}

	if(!wprc_insert_data($new_report)){
		$message['message'] = '举报失败，请稍后重试';
		die(json_encode($message));
	}

	wprc_mail($new_report);
	$message['success'] = 1;
	$message['message'] = '举报已经提交';
	die(json_encode($message));
}
