        ...
        if($this->serverSide()) {
            ...
            if(!isset($_GET['draw'])){
                ...
                if ( isset( $_GET['start'] ) && $_GET['length'] != '-1' ) {
                            $limit = "LIMIT ".addslashes( $_GET['start'] ).", ".
                                addslashes( $_GET['length'] );
                        }
                ...
                if ( isset( $_GET['order'] ) )
                {
                    ...
                    for ( $i=0 ; $i<count( $_GET['order'] ) ; $i++ )
                    {
                        ...
                        {
                            $orderby .= '`'.$aColumns[ $_GET['order'][$i]['column'] ]."`
                                    ".addslashes( $_GET['order'][$i]['dir'] ) .", ";
                        }
                    }
                ...
                if ( !empty( $_GET['search']['value'] ) )
                {
                    ...
                    for ( $i=0 ; $i<count($aColumns) ; $i++ )
                    {
                        if( $_GET['columns'][$i]['searchable'] == "true" ){
                            $search .= '`'.$aColumns[$i]."` LIKE '%".addslashes( $_GET['search']['value'] )."%' OR ";
                        }
                    }
                ...
                for ( $i=0 ; $i<count($aColumns) ; $i++ )
                {
                    if ( ( $_GET['columns'][$i]['searchable'] == "true" ) 
                        ...
                        switch($wdtParameters['filter_types'][$aColumns[$i]]) {
                            case 'number':
                                $search .= '`'.$aColumns[$i]."` = ".$_GET['columns'][$i]['search']['value']." ";
                                break;
                            case 'number-range':
                                list($left, $right) = explode('~', $_GET['columns'][$i]['search']['value']);
                                if($left !== ''){
                                    $left = (float) $left;
                            ...
                            case 'date-range':
                                ...
                                break;
                            case 'select':
                                $search .= '`'.$aColumns[$i]."` = '".addslashes( $_GET['columns'][$i]['search']['value'] )."' ";
                                break;
                            case 'checkbox':
                                $checkboxSearches = explode('|', $_GET['columns'][$i]['search']['value'] );
                                $j = 0;
                                $search .= " (";
                            ...
                            case 'text':
                            default:
                                $search .= '`'.$aColumns[$i]."` LIKE '%".addslashes( $_GET['columns'][$i]['search']['value'] )."%' ";
                        }
                    }
                }
	...
	if($this->isEditable()) {
        ...
        $obj->dataTableParams->bProcessing = false;
     	if($this->serverSide()){
            ...
            $obj->processing = true;
            $obj->dataTableParams->ajax = site_url().'/wp-admin/admin-ajax.php?action=get_wdtable&table_id='.$this->getWpId();
            if( !empty( $wdt_var1 ) ){
                $obj->dataTableParams->ajax .= '&wdt_var1='.urlencode( $wdt_var1 );
            }
            if( !empty( $wdt_var2 ) ){
                $obj->dataTableParams->ajax .= '&wdt_var2='.urlencode( $wdt_var2 );
            }
            if( !empty( $wdt_var3 ) ){
                $obj->dataTableParams->ajax .= '&wdt_var3='.urlencode( $wdt_var3 );
            }
